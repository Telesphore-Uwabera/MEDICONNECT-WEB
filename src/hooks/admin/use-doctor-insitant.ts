...
// NEW ─── Instant consultation toggle
export interface ToggleInstantPayload {
  doctor_id: number;
  active: boolean;
}

export interface ToggleInstantResponse {
  message: string;
  doctor: {
    id: number;
    instant_consultation: boolean;
...
/* PUT /admin/instant-consultations  — toggle instant_consultation on the doctor */
export function useToggleInstantConsultation() {
  const qc = useQueryClient();
  return useMutation<ToggleInstantResponse, Error, ToggleInstantPayload>({
    mutationFn: (payload) =>
      apiFetch("/admin/instant-consultations", { method: "POST", body: payload }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["doctor-consultations"] }),
  });
}
