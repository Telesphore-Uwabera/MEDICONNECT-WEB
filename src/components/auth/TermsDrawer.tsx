...
const TermsSection = ({
  ...
  // (happens when the drawer reopens and the parent resets readSections)
  useEffect(() => {
    if (!isRead) {
  ...
  return (
    <div className="flex h-full flex-col rounded-[6px] border border-border overflow-hidden">
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto px-4 py-3 bg-card text-xs text-muted-foreground leading-relaxed"
      >
        {isLoading && (
...
const TermsDrawer = ({ open, onClose, onAccept }: TermsDrawerProps) => {
  ...
  // Reset read state whenever the drawer opens
  useEffect(() => {
        ...
        <>
          ...
          >
            <motion.div
              ...
              className="w-[90vw] h-[90vh] max-w-5xl bg-card border border-border rounded-[10px] shadow-2xl flex flex-col overflow-hidden"
            >
              {/* Header */}
              ...
              {/* Scrollable content */}
              <div className="flex-1 overflow-y-auto px-5 py-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                {/* Terms & Conditions */}
                <TermsSection
                  title={termsTitle}
                  content={termsContent}
                  isLoading={termsDoc.isLoading}
                  isError={termsDoc.isError}
                  sectionIndex={0}
                ...
                <TermsSection
                  ...
                  isLoading={privacyDoc.isLoading}
                  isError={privacyDoc.isError}
                  sectionIndex={1}
                  readSections={readSections}
                  onRead={onRead}
                />
