            ...
            <div className="sticky top-0 z-10 mt-3 sm:mt-4 bg-background/90 backdrop-blur-md border-b border-border/60 px-3 sm:px-4 py-2.5 flex items-center justify-between gap-2 sm:gap-3">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <p className="text-[11px] text-muted-foreground shrink-0">
                  {isLoading ? (
                    ...
                    <><span className="font-bold text-foreground">{total}</span> {total === 1 ? "patient" : "patients"}</>
                  )}
                  {hasActiveFilters && (
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className="relative hidden sm:block">
                  <SearchInput value={searchInput} onChange={setSearchInput} className="w-48" />
                </div>
            ...
            <div className="p-3 sm:p-4">
              {isError ? (
                ...
                <EmptyState
                  ...
                  action={<button onClick={clearAll} className="text-[11px] text-primary hover:text-primary/80 font-semibold hover:underline transition-colors mt-1">Clear all filters</button>}
                />
              ) : (
                <>
                  {/* Desktop table */}
                  <div className="hidden md:block rounded-sm border border-border/70 bg-card overflow-hidden shadow-sm">
                    <table className="w-full text-[11px]">
                      ...
                      <tbody>
                        {isLoading ? <SkeletonRows /> : sorted.map((p) => (
                  ...
                  <div className="md:hidden flex flex-col gap-2">
                    {isLoading
                      ...
                      : sorted.map((p) => <PatientCard key={p.id} p={p} onManage={setSelected} />)
                    }
                  </div>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/60">
                      ...
                      <div className="flex items-center gap-1.5">
                        <PaginationButton disabled={filters.page <= 1} onClick={() => set("page", filters.page - 1)}>
                          <ChevronLeft className="w-3.5 h-3.5" />
                        </PaginationButton>
                        <PaginationButton disabled={filters.page >= totalPages} onClick={() => set("page", filters.page + 1)}>
...
// ─── Tiny local helpers (too small to extract) ────────────────────────────────
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className={cn("pl-8 pr-3 py-1.5 text-[11px] bg-background border border-border/60 rounded-sm text-foreground outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/50 placeholder:text-muted-foreground/40 transition-all w-full", className)}
      />
      {value && (
        <button onClick={() => onChange("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground/50 hover:text-foreground">
...
function EmptyState({ icon, title, subtitle, action }: { icon: React.ReactNode; title: string; subtitle: string; action?: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 sm:py-24 gap-3 text-center">
      <div className="w-14 h-14 rounded-sm bg-muted/60 flex items-center justify-center border border-border/40">{icon}</div>
      <div>
