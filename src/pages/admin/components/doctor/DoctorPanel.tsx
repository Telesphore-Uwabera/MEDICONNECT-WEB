...
const fmtMonth = (iso?: string | null) =>
  iso ? moment(iso).format("MMM YYYY") : null;
const fmtTime = (iso?: string | null) =>
  iso ? moment(iso).format("HH:mm") : null;
...
const TABS: { id: TabId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "fees", label: "Fees", icon: <CreditCard className="w-3 h-3" /> },
...
const QC_STATUS_VARIANT: Record<
  QCStatus,
  "emerald" | "teal" | "amber" | "red" | "violet" | "default"
> = {
  pending: "amber",
  confirmed: "teal",
  accepted: "teal",
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-[9px] font-semibold uppercase tracking-[0.08em] text-muted-foreground/40">
          {label}
        </span>
      </div>
      <p
        className={cn(
          "text-[11.5px] font-medium text-foreground/85 truncate",
          mono && "font-mono",
        )}
      >
        {value}
...
function SectionEmpty({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-14 gap-3 text-center">
      ...
      <p className="text-[11px] text-muted-foreground/35 max-w-[180px] leading-relaxed">
        {label}
      </p>
    </div>
  );
...
function FieldRow({
  ...
  return (
    <div className="flex gap-3 text-[11px] leading-relaxed">
      <span className="text-muted-foreground/40 shrink-0 w-[68px] font-medium pt-px">
        {label}
      </span>
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 mb-3">
      <span className="text-[9px] font-bold uppercase tracking-[0.1em] text-primary/40">
        {children}
      </span>
    ...
    <div className="p-3.5 rounded-[6px] border border-border/40 bg-card/60 hover:border-primary/25 hover:bg-accent/10 transition-all">
      ...
      <div className="flex items-center gap-1.5 flex-wrap mb-2">
        ...
        {qc.payment_status && (
          <Pill
            variant={
              qc.payment_status === "paid"
                ? "emerald"
                : qc.payment_status === "unpaid"
                  ? "amber"
                  : "default"
            }
          >
          ...
          <SectionEmpty
            label={
              hasActiveFilters
                ...
                : "No quick consultations for this doctor yet"
            }
          />
        ) : (
          <>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {doctor.rating_avg !== undefined && doctor.rating_avg !== null && (
              <InfoTile
                ...
                label="Rating"
                value={`${Number(doctor.rating_avg).toFixed(1)} / 5.0`}
              />
            )}
            <InfoTile
              icon={<Calendar className="w-2.5 h-2.5" />}
              label="Joined"
              value={fmtDate(doctor.created_at) ?? "—"}
            />
            <InfoTile
            ...
            {doctor.consultation_type && (
              <InfoTile
                ...
                label="Consult type"
                value={
                  CONSULTATION_LABELS[doctor.consultation_type] ??
                  doctor.consultation_type
                }
              />
            )}
                  ...
                  <div className="min-w-0">
                    ...
                    {h.address && (
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <p className="text-[10.5px] text-muted-foreground/50 truncate">
                          {h.address}
                        </p>
                      </div>
                    )}
          ...
          <div>
            ...
            <Card className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-[12px] font-semibold text-foreground">
                  Pharmacy #{doctor.pharmacy.id}
                </p>
                <p className="text-[10.5px] text-muted-foreground/50 capitalize">
                  {doctor.pharmacy.status}
                </p>
              </div>
...
function AppointmentsTab({ doctorId }: { doctorId: number }) {
  ...
  const apptStatusVariant = (
    s: string,
  ): "emerald" | "amber" | "red" | "teal" | "violet" | "default" => {
              ...
              {data.data.map((appt) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {appt.insurance && (
                      <Pill variant="primary">
                        {(appt.insurance as { name?: string }).name ??
                          "Insured"}
                      </Pill>
                    )}
                    {appt.daily_room_url && (
...
function EducationTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  if (list.length === 0)
    return <SectionEmpty label="No education records have been added yet" />;
        ...
        {list.map((e, i) => (
          <Card key={e.id} className="flex gap-3">
            <div className="flex flex-col items-center gap-1 shrink-0">
              ...
              {i < list.length - 1 && (
                <div className="w-px flex-1 min-h-[14px] bg-primary/10 my-1" />
              )}
            </div>
            <div className="flex-1 min-w-0 space-y-1 pb-0.5">
              ...
              {(e.start_year || e.end_year) && (
                <FieldRow
                  label="Period"
                  value={`${e.start_year ?? "?"} – ${e.end_year ?? "Present"}`}
                />
              )}
            </div>
          </Card>
...
function ExperienceTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  if (list.length === 0)
    return <SectionEmpty label="No work experience has been added yet" />;
        ...
        {list.map((e, i) => (
          <Card key={e.id} className="flex gap-3">
            <div className="flex flex-col items-center gap-1 shrink-0">
              ...
              {i < list.length - 1 && (
                <div className="w-px flex-1 min-h-[14px] bg-border/25 my-1" />
              )}
            </div>
...
function QualificationsTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  if (list.length === 0)
    return <SectionEmpty label="No credentials have been added yet" />;
            ...
            <div className="flex-1 min-w-0 space-y-1">
              ...
              {q.expires_at && (
                <div className="flex gap-3 text-[11px] leading-relaxed">
                  ...
                  <span
                    className={cn(
                      isExpired(q.expires_at)
                        ? "text-red-500 font-medium"
                        : "text-foreground/70",
                    )}
                  >
                    {fmtDate(q.expires_at)}
                    {isExpired(q.expires_at) && " · Expired"}
                  </span>
                </div>
              )}
              {q.certificate_file && (
                ...
                >
                  <FileText className="w-2.5 h-2.5" /> View certificate{" "}
...
function ScheduleTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  const grouped = DAY_ORDER.reduce<Record<string, typeof raw>>((acc, d) => {
    const slots = raw.filter(
      (a) => (a as unknown as { day_of_week: string }).day_of_week === d,
    );
  ...
  if (raw.length === 0)
    return (
      <SectionEmpty label="No availability schedule has been configured" />
    );
...
function InstantTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  const { data: instantData, isLoading: instantLoading } =
    useGetInstantStatus(doctorId);

  const { data: pausedData, isLoading: pausedLoading } =
        ...
        {isInactive && (
          <div className="flex items-start gap-2.5 p-3.5 rounded-[6px] border border-amber-200/60 dark:border-amber-800/40 bg-amber-50/40 dark:bg-amber-950/8">
            ...
            <p className="text-[11px] text-amber-700 dark:text-amber-400">
              ...
              — toggles will be rejected by the server.
            </p>
          </div>
        )}
          ...
          <div className="flex items-center justify-between px-4 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10.5px] text-muted-foreground/45 mt-0.5">
                  {instantLoading ? "Loading…" : (instantData?.status ?? "—")}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              {!instantLoading && instantData && (
                <Pill
                  variant={
                    instantData.instant_consultation ? "teal" : "default"
                  }
                >
          ...
          <div className="flex items-center justify-between px-4 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10.5px] text-muted-foreground/45 mt-0.5">
                  {pausedLoading ? "Loading…" : (pausedData?.status ?? "—")}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              {!pausedLoading && pausedData && (
                <Pill
                  variant={pausedData.bookings_paused ? "amber" : "emerald"}
                >
...
function CertificationTab({ doctor }: { doctor: ApiDoctor }) {
  ...
  const isTogglingStatus =
    updateMutation.isPending && updateMutation.variables?.status !== undefined;
  const isTogglingAvail =
    updateMutation.isPending &&
    updateMutation.variables?.is_available !== undefined;
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      setRemoveError(
        (err as { message?: string })?.message ?? "Cannot remove at this time.",
      );
    }
  };
        ...
        <div className="flex items-start gap-3 p-4 rounded-[6px] border border-primary/20 bg-accent/20">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground/55 mt-0.5 leading-relaxed">
              Certification doctors review and approve patient medical
              certificates. They handle pending and in-review requests.
            </p>
          </div>
        </div>
            ...
            {isOnTeam && (
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3.5 rounded-[6px] border border-amber-200/60 dark:border-amber-800/40 dark:bg-amber-950/8">
                  ...
                  <p className="text-[24px] font-bold text-amber-700 dark:text-amber-400 tabular-nums leading-none">
                    {entry.pending_count}
                  </p>
                </div>
                <div className="p-3.5 rounded-[6px] border border-primary/20 bg-accent/25 dark:bg-primary/8">
                  ...
                  <p className="text-[24px] font-bold text-primary tabular-nums leading-none">
                    {entry.in_review_count}
                  </p>
                </div>
              </div>
            )}
  ...
  const handleOverride = async () => {
    ...
    try {
      const res = await overrideMutation.mutateAsync({
        ...
        in_person_fee_override: inPersonOverride
          ? Number(inPersonOverride)
          : null,
                ...
                >
                  ...
                  {isEditing ? (
                    ...
                    <div className="px-4 py-2.5 border-t border-border/20">
                      ...
                      >
                        <Pencil className="w-2.5 h-2.5" /> Edit fee tier /
                        status
                      </button>
                    </div>
                  )}
...
function WalletTab({ doctorId }: { doctorId: number }) {
  ...
  const [mode, setMode] = useState<"idle" | "topup" | "deduct" | "delete">(
    "idle",
  );
  ...
  return (
    <ContentWrap>
      <div className="space-y-3">
        ...
        >
          <p className="text-[9px] font-bold uppercase tracking-[0.1em] text-primary/40 mb-2">
            Wallet balance
          </p>
          {hasWallet ? (
            <>
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[12px] font-semibold text-muted-foreground/45">
                  {currency}
                </span>
              </div>
              {wallet.updated_at && (
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-[11.5px] text-muted-foreground/35">
                No wallet found for this doctor
              </p>
            </div>
          )}
        </div>
          ...
          <div className="rounded-[6px] border border-primary/20 bg-accent/10 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="block text-[9px] font-bold uppercase tracking-[0.08em] text-primary/40 mb-1.5">
                  Note{" "}
                  <span className="normal-case tracking-normal font-normal text-muted-foreground/25">
                    (optional)
                  </span>
                </label>
                <input
          ...
          <div className="rounded-[6px] border border-red-200/60 dark:border-red-800/40 bg-red-50/40 dark:bg-red-950/8 p-4 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground/55 mt-0.5 leading-relaxed">
                  All wallet data will be permanently removed and cannot be
                  undone.
                </p>
              </div>
            </div>
            ...
            <div className="flex-shrink-0 border-b border-primary/10 bg-card/40">
              ...
              <div className="flex items-center justify-between px-6 pt-4 pb-3">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground/45 mt-0.5">
                    Review details and manage account status
                  </p>
                </div>
                <button
                  onClick={onClose}
                ...
                <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
                  ...
                  {d.consultation_type && (
                    ...
                    >
                      ...
                      {CONSULTATION_LABELS[d.consultation_type] ??
                        d.consultation_type}
                    </span>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {profileLoading && !fullDoctor ? (
                ...
                <div className="px-6 py-5">
                  ...
                  {tab === "quick_consults" && (
                    <QuickConsultsTab doctorId={d.id} />
                  )}
