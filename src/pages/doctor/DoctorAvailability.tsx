...
//   const displayTime = slot.start_time.slice(0, 5); // "09:00:00" → "09:00"
...
//   } = useGetSlots(
//     fromStr ? { from: fromStr, to: toStr || fromStr } : undefined,
//   );
...
//     // Derive days_of_week from the selected range
...
//             description: `${res.total_days} days · ${res.slots_generated} slots · ${interval}-min intervals`,
//           });
//         },
//         onError: (err) => {
...
//       if (slot.status === "booked") return; // API rejects booked changes
...
//                             // Clamp `from` to today at minimum
//                             const clampedFrom =
...
//                         <div
//                           key={i}
//                           className="shrink-0 w-14 h-16 rounded-sm bg-muted animate-pulse"
//                         />
//                       ))}
//                     </div>
//                   ) : days.length === 0 ? (
...
// /* ─────────────────────────────────────────────
//    DayPickerGroupedByMonth
// ───────────────────────────────────────────── */

// function DayPickerGroupedByMonth({
//   days,
//   selectedDay,
//   onSelectDay,
//   interval,
...
//     const groups: { monthLabel: string; monthKey: string; days: { day: typeof days[0]; index: number }[] }[] = [];
...
//     // Expand the month containing the selected day by default
//     const initial = new Set<string>();
//     if (days[selectedDay]) {
//       initial.add(moment(days[selectedDay].date).format("YYYY-MM"));
//     } else if (groupedByMonth.length > 0) {
//       initial.add(groupedByMonth[0].monthKey);
//     }
//     return initial;
//   });

//   useEffect(() => {
//     // Auto-expand month when selectedDay changes
//     if (days[selectedDay]) {
//       const monthKey = moment(days[selectedDay].date).format("YYYY-MM");
...
//         />
//         <p className="text-[11px] text-muted-foreground">
//           Select a date range and click "Generate schedule" to create slots
//         </p>
//       </div>
//     );
//   }
...
//         <div key={group.monthKey} className="border border-border/40 rounded-sm overflow-hidden">
//           {/* Month header */}
//           <button
...
//           {/* Days in month */}
//           {expandedMonths.has(group.monthKey) && (
//             <div className="flex gap-1.5 overflow-x-auto p-2 scrollbar-thin scrollbar-thumb-border">
//               {group.days.map(({ day, index }) => {
//                 const m = moment(day.date);
//                 const dayAvail = day.slots.filter(
//                   (s) => s.status === "available",
//                 ).length;
//                 const dayBooked = day.slots.filter(
//                   (s) => s.status === "booked",
//                 ).length;
...
//   const [generatedRange, setGeneratedRange] = useState<{ from: string; to: string } | null>(null);
...
//     isFetching: slotsFetching,
//   } = useGetSlots(
//     generatedRange ? { from: generatedRange.from, to: generatedRange.to } : undefined,
//   );
...
//       {
//         onSuccess: (res) => {
//           // Only set the generated range after successful creation
    ...
    >
     ...
     <div className="flex flex-row justify-around items-center w-full">
      ...
      <div className="w-full flex h-[3px] rounded-full overflow-hidden gap-px mt-1">
        {bookedPct > 0 && (
          <div className="bg-primary h-full rounded-l-full" style={{ width: `${bookedPct}%` }} />
        )}
        {availPct > 0 && (
          <div className="bg-emerald-400 h-full" style={{ width: `${availPct}%` }} />
        )}
        {blockedPct > 0 && (
          <div className="bg-muted-foreground/30 h-full" style={{ width: `${blockedPct}%` }} />
        )}
        {reservedPct > 0 && (
          <div className="bg-amber-400 h-full rounded-r-full" style={{ width: `${reservedPct}%` }} />
        )}
      </div>
      ...
      <div className="flex flex-col items-center justify-center py-10 space-y-3 text-center">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground max-w-[260px]">
            Select a date range and click "Generate schedule" to create your availability slots
          </p>
        </div>
      </div>
    );
