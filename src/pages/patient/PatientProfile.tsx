...
//             <div className="w-12 h-12 rounded-full flex items-center justify-center text-xs font-semibold tracking-tight text-destructive-foreground bg-destructive shrink-0 ">
//               {profile.blood_type}
//             </div>
//             <div>
...
//         <div className="rounded-xl border border-border bg-card overflow-hidden  flex flex-col sm:flex-row min-h-[480px]">
//           <UnifiedSidebar
...
import type { PatientProfile as TPatientProfile } from "@/types/patient-profile";
    ...
    <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <SectionCard icon={User} title={t("profile.section.personal", "Personal Information")}>
          <div className="grid grid-cols-2 gap-3">
            <Field label={t("profile.field.dob", "Date of birth")} value={formatDate(profile.date_of_birth)} />
            <Field label={t("profile.field.gender", "Gender")} value={profile.gender.charAt(0).toUpperCase() + profile.gender.slice(1)} />
          </div>
        </SectionCard>
        ...
        <SectionCard icon={HeartPulse} title={t("profile.section.medical", "Medical Information")}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {profile.insurance
                  ? `${profile.insurance.name} · ${profile.insurance.coverage_percentage}% coverage`
                  : "No insurance linked"}
              </p>
            </div>
          </div>
        ...
        <SectionCard icon={Phone} title={t("profile.section.emergency", "Emergency Contact")}>
          <p className="text-xs text-muted-foreground">
            Emergency contact info is managed separately. Use the edit form to update.
          </p>
        </SectionCard>
      </div>
    </div>
...
const PatientProfile = () => {
  ...
  const resolvedMode: Mode = mode ?? (profile ? "view" : "create");
  ...
  const handleDelete = () => {
    ...
    setCurrentStep(0);
    setVisited(new Set([0]));
    toast.info("Profile cleared locally. Re-create to save a new one.");
  };
