          ...
          <div className="rounded-[6px] bg-primary/5 border border-primary/15 px-4 py-2.5 flex items-center justify-between">
            ...
            {outOfStock ? (
              ...
              <div className="flex items-center gap-1.5 h-8 rounded-[6px] border border-primary/30 bg-primary/5 px-2">
                <button onClick={onDec} className="h-5 w-5 rounded-[4px] flex items-center justify-center hover:bg-primary/10 transition-colors text-primary">
                  <Minus className="h-3 w-3" />
                </button>
                <span className="text-[12px] font-bold tabular-nums text-primary w-4 text-center">{cartQty}</span>
                <button onClick={onInc} className="h-5 w-5 rounded-[4px] flex items-center justify-center hover:bg-primary/10 transition-colors text-primary">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0 pr-10">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className={`inline-flex items-center gap-1 text-[9px] font-semibold px-2 py-0.5 rounded-full ${
            ...
            <div className="grid grid-cols-4 gap-x-4 gap-y-1">
              {ph.working_hours.map((h) => (
                <div key={h.id} className="flex items-center justify-between text-[10px]">
                  <span className={`font-semibold w-6 ${h.day_of_week === TODAY ? "text-primary" : "text-muted-foreground"}`}>
...
const MedicineCard = ({ medicine: med, cartQty, outOfStock, onView, onAdd, onInc, onDec }: MedicineCardProps) => (
  <div className={`flex flex-col rounded-[6px] border bg-card transition-all ${
    cartQty > 0
      ? "border-primary/40 shadow-sm"
      : "border-border hover:border-primary/30 hover:shadow-sm"
  }`}>
    {/* {cartQty > 0 && (
      <div className="h-0.5 w-full bg-gradient-primary rounded-t-[6px]" />
    )} */}
    ...
    <div className="p-2.5 flex flex-col gap-2 flex-1">
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[11px] font-semibold leading-tight text-foreground line-clamp-1">
            {med.name}
          </div>
          {med.generic_name && (
      ...
      <div className="flex items-center gap-1 flex-wrap">
        ...
        {med.unit && (
          <span className="text-[9px] text-muted-foreground bg-muted px-1 py-0.5 rounded-[3px]">
            /{med.unit}
          </span>
        )}
        {med.rating != null && (
      ...
      {outOfStock ? (
        ...
        <div className="flex items-center justify-between h-7 rounded-[6px] border border-primary/30 bg-primary/5 px-1">
          <button
            onClick={onDec}
