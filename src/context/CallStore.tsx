...
export type ConnectPhase =
  | "idle" | "setup" | "checking" | "permissions"
  | "connecting" | "ringing" | "connected" | "failed" | "ended";

export type UserRole = "patient" | "doctor";

export interface ChatMessage {
  id: string; from: "me" | "doctor"; text: string; timestamp: number;
}

export interface IncomingRequest {
  id: string; patientName: string; patientAvatar: string;
  reason: string; waitingSince: number; priority: "routine" | "urgent";
}
...
const PATIENT_NAMES    = ["James Okafor","Amara Diallo","Sophie Nguyen","Carlos Reyes","Fatima Al-Hassan","David Chen","Layla Ibrahim","Marcus Thompson"];
const PATIENT_REASONS  = ["Follow-up on blood pressure medication","Persistent headache for 3 days","Annual wellness check","Chest tightness and shortness of breath","Medication refill request","Post-surgery check-in","Fever and sore throat","Lower back pain consultation"];
const PATIENT_AVATARS  = ["JO","AD","SN","CR","FA","DC","LI","MT"];
let requestCounter = 0;

export const CallStoreProvider = ({ children }: { children: ReactNode }) => {
  const [state, setState] = useState<CallState>({
    phase: "idle", role: "patient", doctor: null,
    videoEnabled: true, audioEnabled: true, elapsed: 0, signalStrength: 4,
    minimized: false, dialogOpen: false, messages: [], unreadCount: 0,
    incomingRequests: [], activeRequest: null, activeAppointment: null,
    callNotes: "", appointmentNotes: {},
  });
  ...
  const startTimers = () => {
    timerRef.current = setInterval(
      () => setState((s) => ({ ...s, elapsed: s.elapsed + 1 })), 1000);
    signalRef.current = setInterval(
      () => setState((s) => ({ ...s, signalStrength: Math.floor(Math.random() * 2) + 3 })), 3000);
  };
  ...
  const goToSetup = useCallback((doctor: Doctor) => {
    doctorRef.current = doctor;
    setState((s) => ({ ...s, role: "patient", phase: "setup", doctor, dialogOpen: true, messages: [], unreadCount: 0 }));
  }, []);
  ...
  const runConnectionFlow = useCallback((doctor: Doctor) => {
    ...
    doctorRef.current = doctor;
    setState((s) => ({
      ...s, role: "patient", phase: "checking", doctor,
      elapsed: 0, minimized: false, dialogOpen: true, messages: [], unreadCount: 0,
    }));
    const steps: { phase: ConnectPhase; delay: number }[] = [
      { phase: "permissions", delay: 900  },
      { phase: "connecting",  delay: 1800 },
  ...
  const startCall = useCallback((doctor: Doctor) => runConnectionFlow(doctor), [runConnectionFlow]);
  ...
  const confirmJoin = useCallback(() => {
    ...
    setState((s) => ({
      ...s, phase: "connected",
      messages: [{ id: crypto.randomUUID(), from: "doctor", text: `Hello! I'm ${doctor.name}. How can I help you today?`, timestamp: Date.now() }],
      unreadCount: 1,
    }));
    startTimers();
  }, []);
  ...
  const startScheduledCall = useCallback((appt: AppointmentContext, doctor: Doctor) => {
    clearAll();
    doctorRef.current = doctor;
    setState((s) => ({
      ...s,
  ...
  const updateAppointmentNotes = useCallback((apptId: string, notes: string) => {
    setState((s) => ({
      ...s,
  ...
  const acceptRequest = useCallback((requestId: string) => {
    setState((s) => {
      ...
      if (!req) return s;
      return {
        ...s, role: "doctor", phase: "connected",
        activeRequest: req, activeAppointment: null,
        elapsed: 0, minimized: false, dialogOpen: false,
        incomingRequests: s.incomingRequests.filter((r) => r.id !== requestId),
        messages: [{ id: crypto.randomUUID(), from: "doctor", text: `Hello ${req.patientName.split(" ")[0]}, I'm joining your call now.`, timestamp: Date.now() }],
        unreadCount: 0, callNotes: "",
      };
    });
    clearAll();
  ...
  const declineRequest = useCallback((requestId: string) => {
    setState((s) => ({ ...s, incomingRequests: s.incomingRequests.filter((r) => r.id !== requestId) }));
  }, []);
  ...
  const sendMessage = useCallback((text: string) => {
    ...
    replyRef.current = setTimeout(() => {
      const reply: ChatMessage = {
        id: crypto.randomUUID(), from: "doctor",
        text: DOCTOR_REPLIES[Math.floor(Math.random() * DOCTOR_REPLIES.length)],
        timestamp: Date.now(),
      };
      setState((s) => ({ ...s, messages: [...s.messages, reply], unreadCount: s.unreadCount + 1 }));
  ...
  return (
    <Ctx.Provider value={{
      ...state,
      goToSetup, startCall, confirmJoin, retryCall,
      startScheduledCall, updateAppointmentNotes,
      acceptRequest, declineRequest, simulateIncomingRequest, updateCallNotes,
      endCall, toggleVideo, toggleAudio, setMinimized, setDialogOpen,
      sendMessage, clearUnread,
    }}>
